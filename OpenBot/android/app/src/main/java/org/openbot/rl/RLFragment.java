...
import org.openbot.logging.SensorsDialog;
...
public class RLFragment extends CameraFragment {
    ...
    private long[] actions = {0, 1, 0};
    private long rewards = 0;

    private long done = 0;

    private long[] info = {actions[0], actions[1], actions[2], rewards, done};
        ...
        if (sensorMessenger != null) {
            try {
                sensorMessenger.send(
                        LogDataUtilsRL.generateControlDataMessage(
    ...
    protected void sendVehicleDataToSensorService(long timestamp, String data, int type) {
        if (sensorMessenger != null) {
            try {
                sensorMessenger.send(LogDataUtilsRL.generateVehicleDataMessage(timestamp, data, type));
            } catch (RemoteException e) {
                e.printStackTrace();
            }
        }
    ...
    protected void sendIndicatorToSensorService() {
        if (sensorMessenger != null) {
            try {
                sensorMessenger.send(LogDataUtilsRL.generateIndicatorMessage(vehicle.getIndicator()));
            } catch (RemoteException e) {
                e.printStackTrace();
            }
        }
    ...
    protected void sendInfoToSensorService() { //This is to send the information for the action and reward
        if (sensorMessenger != null) {
            try {
                sensorMessenger.send(LogDataUtilsRL.generateRLMessage(info));
            } catch (RemoteException e) {
                e.printStackTrace();
            }
        }
